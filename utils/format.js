  ...
  for (let i = 0; i < tables.length; i++) {
    ...
    try {
      ...
      item.id = $ulink
        .attr("href")
        .replace(
          /\/|\.|\:|https|www|ygdy8|dytt8|com|net|html|gndy|jddy|yddy|dyzz/g,
          ""
        );
  ...
  let getMovies = (pathNum) => {
    ...
    for (let i = 0; i < length; i++) {
      if ($(`${paths[pathNum].path} tr:nth-child(${i + 1}) a`).length > 1) {
        let item = {};
        item.id = $(
          `${paths[pathNum].path} tr:nth-child(${i + 1}) a:nth-child(2)`
        )
          .attr("href")
          .replace(
            /\/|\.|\:|https|www|ygdy8|dytt8|com|net|html|gndy|jddy|yddy|dyzz|.html/g,
            ""
          );
        item.title = $(
          `${paths[pathNum].path} table tr:nth-child(${i + 1}) a:nth-child(2)`
        ).text();
        item.url =
          host +
          $(
...
let formatDetails = function (res) {
  ...
  if (!json.id) {
    json.id = json.url.replace(
      /\/|\.|\:|https|www|ygdy8|dytt8|com|net|html|gndy|dyzz|.html/g,
      ""
    );
  }
  if (!json.date) {
...
let formatDouban = function (res, data) {
  ...
  if (res.request.uri.host !== "www.imdb.com") {
    ...
    json.douban_user =
      $(".rating_self .rating_right .rating_sum span").text() || "0";
    json.imdb_id = $("#info")
      .text()
      .replace(/\s*/g, "")
      .match(/IMDb:tt[\d]*/g)
    ...
    json.release = $("#info")
      ...
      ? $("#info")
          ...
          .replace(/\s*/g, "")
          .match(
            /(上映日期:|首播:)[\s|\S]*?(片长:|又名:|IMDb:|集数:|季数:)/g
          )[0]
    ...
    json.areas = $("#info")
      ...
      .replace(/\s*/g, "")
      .match(
        /(制片国家\/地区:)[\s|\S]*?(语言:|上映日期:|片长:又名:|IMDb:|集数:|季数:)/g
      )
      ? $("#info")
          .text()
          .replace(/\s*/g, "")
          ...
          .replace(
            /制片国家\/地区:|语言:|上映日期:|片长:又名:|IMDb:|集数:|季数:|分钟/g,
            ""
          )
      : "";
    json.language = $("#info")
      ...
      ? $("#info")
          ...
          .replace(/\s*/g, "")
          .match(
            /(语言:)[\s|\S]*?(上映日期:|片长:|又名:|IMDb:|集数:|季数:)/g
          )[0]
    ...
    json.category = $("#info")
      ...
      : "";
    json.introduction = $("#link-report-intra")
      .text()
      .replace(/\s*/g, "")
      .replace(/©豆瓣/g, "");
    // 二次处理分隔符
    ...
    if ($) {
      ...
      if (json.imdb_user.indexOf("K") != -1) {
        json.imdb_user =
          (json.imdb_user.replace(/K/g, "") * 1000).toString() || "0";
      } else if (json.imdb_user.indexOf("M") != -1) {
        json.imdb_user =
          (json.imdb_user.replace(/M/g, "") * 1000000).toString() || "0";
      }
    } else {
      json.imdb = "";
      json.imdb_user = "";
...
let updateDouban = function (res, data) {
  ...
  if ($) {
    ...
    json.introduction =
      json.introduction ||
      $("#link-report-intra").text().replace(/\s*/g, "").replace(/©豆瓣/g, "");
